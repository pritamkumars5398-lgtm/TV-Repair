...
export default function CustomerLoginPage() {
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await authApi.verifyOtp(phone, otp, 'customer');
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-8 text-3xl font-extrabold text-slate-800 tracking-tight">Customer Login</h1>
          <p className="mt-2 text-sm text-slate-500 font-medium">Track your repairs and manage your account</p>
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/50 p-8 sm:p-10 relative overflow-hidden">
          ...
          {step === 'phone' ? (
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
              <div className="text-center mb-6">
                ...
                <p className="text-sm font-medium text-slate-600">OTP sent to <span className="font-bold text-slate-800">+91 {phone}</span></p>
                <button type="button" onClick={() => setStep('phone')} className="text-xs font-bold text-primary-600 hover:text-cyan-600 transition-colors mt-2">Change number</button>
              </div>
              <div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-primary-600 to-cyan-600 hover:from-primary-500 hover:to-cyan-500 text-white font-bold py-3.5 rounded-2xl transition-all shadow-lg shadow-cyan-500/25 hover:shadow-xl hover:shadow-cyan-500/30 hover:-translate-y-0.5 disabled:opacity-60 disabled:hover:translate-y-0"
              >
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><ArrowRight className="h-5 w-5" /> Verify & Login</>}
              </button>
            </form>
          )}
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 text-center">
            <p className="text-sm font-medium text-slate-500">
              New customer?{' '}
              <Link href="/portal/register" className="text-primary-600 font-bold hover:text-cyan-600 transition-colors">
                Create an account
              </Link>
            </p>
          </div>
