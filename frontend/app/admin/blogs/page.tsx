...
export default function AdminBlogsPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-7xl mx-auto px-4 sm:px-6">
      <div className="rounded-[1rem] border border-slate-200 bg-white p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Blog Management</h1>
            <p className="mt-1.5 text-sm text-slate-500 max-w-2xl">Manage posts, upload images, and keep the admin blog table neat and easy to scan.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                className="w-full sm:w-auto border border-slate-200 bg-slate-50 rounded-xl px-3 py-2 text-xs font-semibold text-slate-700 focus:outline-none"
              >
            ...
            <button 
              ...
              className="inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-sm transition-all"
            >
        ...
        <div className="mt-6 grid gap-4 grid-cols-3">
          <div className="rounded-xl sm:rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4 text-center sm:text-left">
            <p className="text-[9px] sm:text-[11px] uppercase tracking-[0.12em] sm:tracking-[0.24em] text-slate-500">Total posts</p>
            <p className="mt-1 sm:mt-3 text-lg sm:text-3xl font-semibold text-slate-900">{allBlogs.length}</p>
          </div>
          <div className="rounded-xl sm:rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4 text-center sm:text-left">
            <p className="text-[9px] sm:text-[11px] uppercase tracking-[0.12em] sm:tracking-[0.24em] text-slate-500">Drafts</p>
            <p className="mt-1 sm:mt-3 text-lg sm:text-3xl font-semibold text-slate-900">{allBlogs.filter((item: any) => item.status === 'DRAFT').length}</p>
          </div>
          <div className="rounded-xl sm:rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4 text-center sm:text-left">
            <p className="text-[9px] sm:text-[11px] uppercase tracking-[0.12em] sm:tracking-[0.24em] text-slate-500">Featured</p>
            <p className="mt-1 sm:mt-3 text-lg sm:text-3xl font-semibold text-slate-900">{allBlogs.filter((item: any) => item.featured).length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[1rem] border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="block sm:hidden divide-y divide-slate-100">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-slate-400 font-semibold text-xs">No blogs found.</div>
          ) : (
            blogs.map((blog: any) => (
              <div key={blog.id} className="p-4 flex gap-4 items-center hover:bg-slate-50 transition-colors">
                <img 
                  src={blog.imageUrl || 'https://images.unsplash.com/photo-1514489383739-55b8e387a112?w=80&h=80&fit=crop'} 
                  alt="blog" 
                  className="h-12 w-12 rounded-lg object-cover shrink-0" 
                />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-slate-900 truncate">{blog.title}</p>
                  <p className="text-[10px] text-slate-500 mt-1 font-medium">{blog.category}</p>
                </div>
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <button 
                    onClick={() => saveMutation.mutate({ ...blog, status: blog.status === 'APPROVED' ? 'DRAFT' : 'APPROVED' })}
                    disabled={saveMutation.isPending}
                    className={`px-2 py-0.5 rounded text-[10px] font-semibold border transition-all ${blog.status === 'APPROVED' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
                    title="Click to toggle status"
                  >
                    {blog.status}
                  </button>
                  <div className="flex gap-1.5">
                    <button onClick={() => handleEdit(blog)} className="p-1 text-slate-600 hover:bg-slate-100 rounded transition" title="Edit"><Edit className="h-3.5 w-3.5" /></button>
                    <button onClick={() => { if(confirm('Are you sure?')) deleteMutation.mutate(blog.id); }} className="p-1 text-red-600 hover:bg-red-100 rounded transition" title="Delete"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </div>
              </div>
            ))
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in">
          <div className="bg-white rounded-xl border border-slate-200 shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto p-5">
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
              <h2 className="text-sm font-bold text-slate-800">{editingBlog ? 'Edit Blog' : 'New Blog'}</h2>
              <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="border border-dashed border-slate-300 rounded-lg bg-slate-50 p-3">
                  <label htmlFor="blog-image" className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded cursor-pointer hover:bg-slate-50 transition text-sm font-medium text-slate-700 w-fit">
